...
# Read in PSM data
lfq_all_oops_rnase_pep <- read.delim(
  here::here("data-raw/RBPMap_RNase_pilot_3/LFQ_OOPS_RNase_MetChloroform-(1)_PeptideGroups.txt")
)


# Output .rda file
usethis::use_data(lfq_all_oops_rnase_pep, overwrite=TRUE)

lfq_all_oops_rnase_psm <- read.delim(
  here::here("data-raw/RBPMap_RNase_pilot_3/LFQ_OOPS_RNase_MetChloroform-(1)_PSMs.txt")
)

# Output .rda file
usethis::use_data(lfq_all_oops_rnase_psm, overwrite=TRUE)
